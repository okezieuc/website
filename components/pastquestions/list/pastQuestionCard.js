...
export default function PastQuestionCard({ questionid, questionData }) {
  return (
    <Link href={`/pastquestions/${questionid}`}>
      <a>
        <div className="bg-white hover:bg-gray-50 hover:shadow-xl p-4 md:p-8 mb-8 md:mb-20 border rounded-lg shadow-lg transition">
          <div className="mb-4">
            {questionData.prompt ? (
            ...
            <div className="text-lg md:text-3xl">
              <Link href={`/pastquestions/${questionid}`}>
                <a>
                  <PastQuestionBlockContent body={questionData.question} />
                </a>
              </Link>
            </div>
          </div>
...
const PastQuestionCardOption = ({ option, blocks }) => (
  <div className="flex my-2 md:my-4">
    <div className="text-md md:text-xl">{option}.</div>
    <div className="text-md md:text-2xl ml-4 md:ml-4">
      <PastQuestionBlockContent body={blocks} />
    </div>
  </div>
);
